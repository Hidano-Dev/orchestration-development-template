...
{ git submodule status --recursive; git submodule foreach --recursive --quiet 'echo "== $displaypath"; git rev-parse HEAD; git status --porcelain; git ls-files -v; git ls-files --stage; git diff HEAD | sha256sum; git ls-files -z | while IFS= read -r -d "" f; do if [ -L "$f" ]; then stat -c "%N %F %a" -- "$f"; elif [ -f "$f" ]; then stat -c "%N %F %a" -- "$f"; sha256sum -- "$f"; fi; done'; } > "$VAL_TMP/submodules-before.txt"
...
{ git submodule status --recursive; git submodule foreach --recursive --quiet 'echo "== $displaypath"; git rev-parse HEAD; git status --porcelain; git ls-files -v; git ls-files --stage; git diff HEAD | sha256sum; git ls-files -z | while IFS= read -r -d "" f; do if [ -L "$f" ]; then stat -c "%N %F %a" -- "$f"; elif [ -f "$f" ]; then stat -c "%N %F %a" -- "$f"; sha256sum -- "$f"; fi; done'; } > "$VAL_TMP/submodules-after.txt"
...
>   - **submodule も独立監査**（`SUBMODULES_DIFF`）: `submodule.<name>.ignore=all` が設定されていると外側の status / diff に submodule 内の変更が一切現れないため、`git submodule status --recursive` と各 submodule 内での superproject と同等の記録 — `rev-parse HEAD` / `status --porcelain` / `ls-files -v`（index フラグ）/ `ls-files --stage`（blob ID）/ diff ハッシュ / 追跡ファイルごとの stat + 内容ハッシュ — を前後比較する（submodule 内の `assume-unchanged` / `skip-worktree` による隠蔽もこれで検出できる）。
